...
module attributes {gpu.container_module} {
    ...
    func.func @main() {
        ...
        // Allocate host memory for the keys and values of the tables
        ...
        //num_tuples = table_x_rows + 1, to store the number of tuples in first node
        %num_tuples = arith.addi %table_x_rows, %cidx_1 : index 

        // Allocate the linked list
        %h_linked_list = memref.alloc(%num_tuples, %cidx_3) : memref<?x?xi32>

        // Convert table_x_rows to i32
        %table_x_rows_i32 = arith.index_cast %table_x_rows : index to i32

        // Store the number of tuples in the first node, key = -1
        memref.store %ci32_neg1, %h_linked_list[%cidx_0, %cidx_0] : memref<?x?xi32>
        memref.store %table_x_rows_i32, %h_linked_list[%cidx_0, %cidx_1] : memref<?x?xi32>
        memref.store %ci32_neg1, %h_linked_list[%cidx_0, %cidx_1] : memref<?x?xi32>


        // Allocate the hash table
        %h_table = memref.alloc(%ht_size, %cidx_2) : memref<?x?xi32>

        // store (hash_val,null) in all the hash table entries 
        scf.for %i = %cidx_0 to %ht_size step %cidx_1 {
            %i_i32 = arith.index_cast %i : index to i32
            memref.store %i_i32, %h_table[%i, %cidx_0] : memref<?x?xi32>
            memref.store %ci32_neg1, %h_table[%i, %cidx_1] : memref<?x?xi32>
        }

        // Allocate both the linked list and the hash table in device memory
        %d_linked_list = gpu.alloc(%num_tuples, %cidx_3) : memref<?x?xi32>
        gpu.memcpy %d_linked_list, %h_linked_list : memref<?x?xi32>, memref<?x?xi32>

        %d_table = gpu.alloc(%ht_size, %cidx_2) : memref<?x?xi32>
        gpu.memcpy %d_table, %h_table : memref<?x?xi32>, memref<?x?xi32>
        ...
        %d_free_index = gpu.alloc() : memref<i32>

        //store 1 in the global block offset
        %h_free_index= memref.alloc() : memref<i32>
        memref.store %ci32_1, %h_free_index[] : memref<i32>
        gpu.memcpy %d_free_index, %h_free_index : memref<i32>, memref<i32>
